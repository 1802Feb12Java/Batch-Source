...
for(var i=0; i<allTags.length; i++){
    if(allTags[i].innerHTML == "USA"){//searches what is between all tags for "USA"
        result = allTags[i];
    }
}
...
function getPeopleInSales(){
    var allTableRows = document.getElementsByTagName("tr");//table where  names and departments lives. 
    for(var i=0; i<allTableRows.length; i++){
        if(allTableRows[i].children[1].textContent == "Sales"){//if dept == sales...
            console.log(allTableRows[i].children[0].textContent);//put name into concole log
        }
    }
}
...
for(var i=0; i<allAnchors.length; i++){
    ...
    for(var j=0; j<anchorChildren.length; j++){
        if(anchorChildren[j].nodeName == "SPAN"){    //span is 1 apparently
            anchorSpanChildren.push(allAnchors[i]); //adds parent to list if <a> tag has child
        }
    }
}
...
function getCustomAttribute(){
    var elementsWithCustom = [];//array of custom elements
    for(var i=0; i<allTags.length; i++){
        if(allTags[i].hasAttribute("data-customAttr")){//if tag is custom...
            elementsWithCustom.push(allTags[i]);//put into array
        }
    }
...
for(var i=0; i<elementsWithCustom.length; i++){
    console.log(elementsWithCustom[i].getAttribute("data-customAttr"));//prints to console the custom
}
console.log("\n");
...
for(var i=0; i<favColorList.length; i++){
    ...
    favColorList[i].onchange = function() {
        ...
        if(lastChoice != null)  //only alert if there was a choice before, otherwise it asks if you like blue more than undefined. 
...
for(var i=0; i<allEmps.length; i++){
    allEmps[i].onmouseover = function(){
        if(this.style.color == "black" || this.style.color == ""){
            this.style.color = "white";//wont actually remove it, just color it the same as the background. 
        }
...
walkTheDOM(x, printTextContents);//prints what is in the a tag... WOO ROLL TIDE!