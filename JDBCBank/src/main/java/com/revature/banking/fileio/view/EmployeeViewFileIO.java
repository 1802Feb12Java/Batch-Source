...
public class EmployeeViewFileIO extends UserViewFileIO {
	private static final long serialVersionUID = -2098377206089222431L;
	public EmployeeViewFileIO() {
	}
	public void home(EmployeeFileIO employee, Scanner scan) {
		//***ACCESS VIEW ONLY**
		//	account information
		//	Account balances
...
//		}	
	...
	public void viewCustomerInfo(EmployeeFileIO employee, Scanner scan) {
		List<CustomerFileIO> customers = bank.getCustomers();
		if (customers.size()==0) {
			System.out.println("There are no customers at this time");
		}
		for (CustomerFileIO customer: customers) {
			System.out.println(customer.toString());
		}
		
	}
	public void viewApplicationsEmployee(EmployeeFileIO employee, Scanner scan) {
		// Print all applications
		if(this.bank.getNumApplications()==0) {
			System.out.println("There are no applications at this time");
			return;
		}
		List<ApplicationFileIO> applications = bank.getApplications();
		...
		ApplicationFileIO select = applications.get(choice-1);
		...
		if(employee instanceof AdminFileIO) {
			this.home((AdminFileIO) employee, scan);
		}
	}
}
