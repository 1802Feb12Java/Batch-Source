...
public class CustomerViewFileIO extends UserViewFileIO {
	private static final long serialVersionUID = -3558653106504986476L;
	public CustomerViewFileIO() {
	}
	public void viewCustomerInfo(CustomerFileIO customer, Scanner scan) {
		System.out.println(customer.toString());
		
	}
	public void home(CustomerFileIO customer, Scanner scan){
	...
	public void viewApplicationsCustomer(CustomerFileIO customer, Scanner scan) {
		List<ApplicationFileIO> customerApplications = bank.getApplicationsByCustomerId(customer.getUserId());
		if(customerApplications.size() ==0) 
			System.out.println("There are no applications at this time");
		else
			for(ApplicationFileIO application: customerApplications) 
				System.out.println(application.getApplicationId()+ "\t"+ application.getStatus());		
	}
	public CustomerFileIO registerCustomer(Scanner scan) {
		...
		CustomerFileIO customer = bank.registerCustomer(username, password, name[0], name[1], email);
	...
	public void customerApply(CustomerFileIO customer, Scanner scan) {
...
//		bank.addNumApplications();
		...
		while (!confirm) {
			...
			if (choice2.equals("yes")) {
					...
					if (choice2.equals("yes")) {
						...
						while(!confirm3) {
							...
							if(this.bank.customerIdExists(secondaryCustomerID)) {
								CustomerFileIO secondaryCustomer = bank.getCustomerById(secondaryCustomerID);
						...
						confirm2 = true;
						CustomerFileIO secondary = this.registerCustomer(scan);
				...
				confirm=true;
				CustomerFileIO secondaryCustomer = null;
				this.bank.customerApply(customer, secondaryCustomer);
				System.out.println("Application completed. An employee will check on your application and"
						+ " get back to you shortly. Thank you\n");
			}else 
	...
	public void modAccountBalance(CustomerFileIO customer, Scanner scan) {
		...
		List<AccountFileIO> accounts = bank.getAccountsByCustomerId(customer.getUserId());
		if(accounts.size()!=0) {
			...
			if(bank.getNumAccounts()!=0) {
				...
				if(choice==2)
					...
					AccountFileIO account = bank.getAccountByAccountId(choice);
					System.out.println("Enter an amount");
					success = false;
					double amount =0;
					while(!success) {
