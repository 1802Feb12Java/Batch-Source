...
public  class CustomerAccountDAOImp implements CustomerAccountDAO{
	...
	public void deleteCustomer(CustomerAccount customer) {
		...
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement("DELETE FROM JointAccount WHERE CustomerID = ?");			
			ps.setInt(1, customer.getCustomerID());
			ps.executeQuery();
			
			ps = conn.prepareStatement("DELETE FROM CustomerAccount WHERE CustomerID = ?");
			ps.setInt(1, customer.getCustomerID());
			ps.executeQuery();
