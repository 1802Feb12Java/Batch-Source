...
public class UserView implements Serializable {
	...
	public void welcome(Scanner scan) {
		...
		if(choice ==1) {
			...
			new CustomerView().registerCustomer(scan);
		}else if(choice == 3) {
			scan.close();
			System.exit(0);
		}else {
		...
		while(!success) {
			...
			if (user.checkPassword(password)) {
				...
				if(user.getRole().equals("Admin")) {
						new AdminView().home((Admin) user, scan);
				}else if (user.getRole().equals("Employee")) {
						new EmployeeView().home((Employee) user, scan);
				}else if (user.getRole().equals("Customer")) {
						new CustomerView().home((Customer) user, scan);
				}
			}else
