...
CREATE OR REPLACE FUNCTION get_receipt(id INTEGER) RETURNS BYTEA AS $$
    DECLARE receipt BYTEA;
BEGIN
    SELECT r_receipt INTO receipt 
        FROM ers_reimbursements
        WHERE r_id = id;
...
author_id INTEGER, r_type_id INTEGER, r_status_id INTEGER) RETURNS VOID AS $$
BEGIN
    INSERT INTO ers_reimbursements(
            r_amount, r_description, r_submitted, u_id_author, 
            rt_type, rt_status)
    VALUES(amount::MONEY, description, submitted, author_id, r_type_id, r_status_id);
END;
$$ LANGUAGE plpgsql;
...
-- Updates a receipt.
CREATE OR REPLACE FUNCTION update_receipt(id INTEGER, receipt BYTEA) 
RETURNS VOID AS $$
BEGIN
    UPDATE ers_reimbursements
